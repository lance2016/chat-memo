...
| 🛠️ 工具管理 | 在设置页查看工具能力、依赖状态，并按工具组启用或关闭；改动会影响后续模型请求 |
...
Compose 默认会同时启动数据库、API、前端和 Phoenix；API 使用源码热重载，
不需要额外的 profile 或启动命令。生产部署或需要避免代码变更重置后台 ticker 时，
再在 `.env` 中显式设置 `RELOAD=0`。
...
如果要临时使用 ChatGPT/Codex 订阅转换出的本地 Responses API，可先启动
`cliproxyapi`，再在启动后端的同一 shell 中设置：

```bash
export OPENAI_BASE_URL=http://127.0.0.1:8317/v1
export OPENAI_API_KEY=sk-your-local-cliproxyapi-key
```

应用会自动发现「OpenAI via Codex」模型服务，并加入代理返回的这组模型；默认使用
`gpt-5.6-luna` 作为初始兜底。正常使用时应在设置页从模型目录选择聊天默认模型，
不需要配置 `OPENAI_MODEL`。图片模型会显示在目录中，但不作为普通聊天模型启用。
该代理只应绑定本机或受保护的可信网络。
...
| 设置页 | 个人资料与外观、模型服务与档案、助手规则、工具、技能、记忆整理、通知、TTS/ASR 等配置 |
...
- [ ] 补充生产运行监控、异常告警和长期趋势面板
- [ ] 分阶段拆分大型前端页面与样式文件，扩大关键交互的自动化回归覆盖

后台任务继续在单实例 API 进程内运行并复用现有 PostgreSQL；当前规模不引入独立 worker、Redis 或消息队列。更完整的阶段计划见[开发路线](docs/roadmap.md)。
