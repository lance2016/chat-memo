...
export function TimelinePage() {
  ...
  const [grouped, completedGroups] = useMemo(() => {
    ...
    return [[...active.entries()], [...completed.entries()]];
  }, [items]);
  ...
  return <div className="timeline-shell"><main className="timeline-content">
    <header className="timeline-heading"><div><span className="eyebrow">{t("timeline.eyebrow")}</span><h1>{t("timeline.title")}</h1><p>{t("timeline.description")}</p></div><button className="primary-button timeline-add-button" onClick={() => setComposerOpen(true)}><Plus size={14} />{t("timeline.add")}</button></header>
    ...
    {loading ? <div className="timeline-empty"><RefreshCw className="spin" size={20} /><strong>{t("timeline.loading")}</strong></div> : view === "month" ? <section className="timeline-calendar"><div className="timeline-weekdays">{Array.from({ length: 7 }, (_, index) => new Intl.DateTimeFormat(locale, { weekday: "short" }).format(addDays(new Date(2024, 0, 1), index))).map((day) => <span key={day}>{day}</span>)}</div><div className="timeline-calendar-grid">{monthDays.map((day) => { const key = dateKey(day); const dayItems = items.filter((item) => dateKey(item.starts_at) === key); const currentMonth = day.getMonth() === cursor.getMonth(); return <div className={`timeline-day ${currentMonth ? "" : "outside"} ${key === dateKey(new Date()) ? "today" : ""}`} key={key}><time>{day.getDate()}</time><div>{dayItems.slice(0, 3).map((item) => <span className={`kind-${item.kind}`} title={item.title} key={item.id}>{item.title}</span>)}</div>{dayItems.length > 3 && <small>{t("timeline.more", { count: dayItems.length - 3 })}</small>}</div>; })}</div></section> : <>
      ...
      {completedGroups.length > 0 && <details className="timeline-completed"><summary><CheckCircle2 size={14} /><span>{t("timeline.completed.title")}</span><b>{completedGroups.reduce((count, [, dayItems]) => count + dayItems.length, 0)}</b></summary><section className="timeline-list">{completedGroups.map(([key, dayItems]) => <div className="timeline-day-group" key={key}><div className="timeline-date"><CalendarDays size={15} /><div><strong>{dayTitle(key, locale, t("timeline.today"), t("timeline.tomorrow"))}</strong><span>{key}</span></div></div><div className="timeline-day-items">{dayItems.map((item) => <TimelineCard item={item} busy={busyId === item.id} onStatus={(status) => void changeStatus(item, status)} onDelete={() => setDeleteTarget(item)} key={item.id} />)}</div></div>)}</section></details>}
    </>}
