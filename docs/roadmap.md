...
### 8. 附件基座（8 / 9 / 10 的共同前置，先做这个）

「文件上传」和「图片识别」是同一件事的两半：都要能把一个文件从浏览器送到后端、
存住、挂到某条消息上、之后每轮都能取回。**分开做会长出两套上传链路**，
而这个仓库已经有过一次同类教训（工具目录曾经是第二份手写清单，见 internals「加新工具」）。

- `attachments` 表：`id / conversation_id / message_id / kind(file|image) / filename /
  mime / bytes / sha256 / created_at`。**正文不进数据库，落磁盘**
- 存储走挂载目录（`ATTACHMENTS_PATH`，同 `SKILLS_PATH` 的做法：可写挂载 + ENV_ONLY）。
  理由：`pg_dump` 是备份的主路径，几十 MB 的 blob 塞进去会让每天的 dump 和 WAL 一起涨，
  而 14 份轮换会把这个代价乘以 14。⚠️ 代价是 `app/backup.py` **必须同步覆盖这个目录**，
  否则恢复演练会漏掉附件 —— 演练手册（backup.md）也要跟着更新
- 去重按 `sha256`：同一张图反复贴不该占两份，也不该重复算一次视觉描述（见 9）
- 消息里存引用而不是内容（`{"type": "attachment_ref", "id": ...}`），
  和「消息必须整块存整块回传」的纪律一致
- 上限、类型白名单、`validate_path` 复用 —— 文件名来自浏览器，同样不可信

**做完这条再动 9 和 10。**

### 9. 图片识别：路由到视觉档案，不是加一个工具

DeepSeek 看不了图，但**解法不该是加个 `recognize_image` 工具**。工具方案的问题是
模型自己看不见图，只读得到一段别人写的描述：用户追问「第三行那个数字是多少」时，
工具得带着这句话重新问视觉模型，而视觉模型没有对话上下文，不知道「第三行」指什么。
多轮追问必然退化，而这恰恰是图片最常见的用法。

**基建其实已经在了**：模型目录的 `capabilities` 里 `vision` 这个字段前端类型已经声明
（`frontend/lib/types.ts:16`），后端 `profile.capabilities` 是自由 dict —— 只是至今没人读它。

所以分支点放在 `ModelTarget` 的 capability 上：

- `vision=true`（Claude / Qwen-VL / GLM-4V）→ image block 原样发，走各自协议的格式
- `vision=false`（DeepSeek）→ **进 agent loop 之前**把图交给模型目录里另配的一个
  「视觉档案」（硅基流动的 Qwen-VL 一类，和标题模型同一条免费链路），换成结构化描述
  + OCR 文本，**存进那条消息**。只算一次，之后每轮复用，不重复烧 token

这么切的好处是：哪天把聊天模型换成 Claude，**零改动**自动升级成原生视觉 ——
因为判据是 target 的能力，不是代码里写死的厂商名。这正是 internals「加新模型」那条铁律
（`provider == "anthropic"` 只允许出现在 `ModelTarget.from_settings()` 一处）。

`image_ask` 工具仍然值得有，但作为**补充**而不是主路径：模型觉得那段描述不够用时，
带着具体问题重新问一次视觉模型。默认路径便宜，深入追问也不瞎。

前置：8。三条里**这条最先做** —— 改动最小、每天都用得上。
