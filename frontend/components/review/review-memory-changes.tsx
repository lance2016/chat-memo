...
export function ReviewMemoryChanges({ changes, onRestore, error }: { changes: MemoryVersion[]; onRestore: (change: MemoryVersion) => void; error?: string }) {
  ...
  return <section className="review-card review-memory-card"><div className="card-heading"><div><span className="card-kicker">{t("review.memory.kicker")}</span><h2>{t("review.memory.title")}</h2><p className="card-description">{t("review.memory.description")}</p></div><span className="count-pill">{error ? "—" : changes.length}</span></div>{error ? <div className="card-state card-state-error">{error}</div> : changes.length ? <div className="memory-groups">{groups.map((group) => { const latest = group.changes[0]; const expanded = expandedPath === group.path; const actorList = Array.from(new Set(group.changes.map((item) => actorLabel(item.actor)))).join(" · "); return <section className={`memory-group ${expanded ? "expanded" : ""}`} key={group.path}><div className="memory-group-header"><span className={`memory-operation operation-${latest.operation}`}>{operationIcon(latest.operation)}</span><div className="memory-change-copy"><div><Link href={memoryLink(latest)} className="memory-change-path">{group.path}</Link><span className="memory-group-count">{t("review.memory.times", { count: group.changes.length })}</span></div><span className="memory-change-meta">{t("review.memory.latest", { operation: operationLabel(latest.operation), time: formatTime(latest.created_at), actors: actorList })}</span></div><button className="change-expand memory-group-toggle" aria-label={`${expanded ? t("review.memory.collapse") : t("review.memory.expand")}${group.path}`} aria-expanded={expanded} onClick={() => setExpandedPath(expanded ? null : group.path)}><ChevronDown size={15} /></button></div>{expanded && <div className="memory-group-events">{group.changes.map((change, index) => { const eventExpanded = expandedId === change.id; const previous = group.changes[index + 1]; return <div className={`memory-group-event ${eventExpanded ? "expanded" : ""}`} key={change.id}><div className="memory-event-row"><span className={`memory-operation operation-${change.operation}`}>{operationIcon(change.operation)}</span><div className="memory-event-copy"><span className={`actor-badge actor-${change.actor}`}>{actorLabel(change.actor)}</span><span className="memory-change-meta">{operationLabel(change.operation)} · {formatTime(change.created_at)}</span></div><button className="change-expand" aria-label={`${eventExpanded ? t("review.memory.collapse") : t("review.memory.expand")}${group.path} ${formatTime(change.created_at)}`} onClick={() => setExpandedId(eventExpanded ? null : change.id)}><ChevronDown size={14} /></button>{change.operation === "deleted" && <button className="restore-button" onClick={() => onRestore(change)}><RotateCcw size={12} />{t("review.memory.restore")}</button>}</div>{eventExpanded && <div className="memory-change-detail"><Snapshot before={previous?.content} after={change.content} /></div>}</div>; })}</div>}</section>; })}</div> : <div className="card-empty compact-empty"><strong>{t("review.memory.emptyTitle")}</strong><span>{t("review.memory.emptyDescription")}</span></div>}<div className="change-summary"><FileText size={13} />{t("review.memory.summary", { files: groups.length, chat: actorCounts.chat, consolidation: actorCounts.consolidation, manual: actorCounts.manual })}</div></section>;
}
