...
| 💬 聊天 | 🧠 记忆 | 🌅 每日回顾 | 🗓️ 时间线 | ⚙️ 设置 |
|---|---|---|---|---|
| 流式回答、思考折叠、记忆工具状态，支持编辑重发、重新生成与语音播放 | 文件树 + 编辑器，记忆全文可读写，历史版本可 diff 和回滚 | 按天浏览会话摘要、记忆变更和 token 用量，也可手动触发整理 | 从对话提取会议、旅行、生日和待办，按今天、最近或月历查看 | 在界面中调整模型、思考、语音、外观和自定义指令，立即生效 |
...
五个页面共用顶部导航与全局搜索（`Cmd/Ctrl + K`）。记忆不是黑盒：你可以在界面中看到模型记了什么、
...
（一天一句话 + 几条收获，一天只有一行，重跑是覆盖）和 `open_loops`（悬而未决的事，
跨天存活，下一次整理时由模型判断是否闭环）。这两样不进 prompt，只服务于回看。
...
| `/timeline` 时间线页 | 查看今天、最近 30 天和月历事项，可确认、完成或手动添加 |
